...
Script Purpose:
	...
	after data cleansing and transformation.
	It performs the following actions:
	- Truncates the 'silver' tables before loading data.
	- Uses the 'Insert Into' command to load data from the 'bronze' schema after data 
...
BEGIN
	...
	BEGIN TRY
		...
		--==============================================================================================================
			...
			END AS sls_order_dt,        -- handling Null and Missing month,year and date characters, also changing datatype from INT to DATE
			CASE 
				WHEN sls_ship_dt = 0 OR LEN(sls_ship_dt) != 8 THEN NULL
				ELSE CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
			END AS sls_ship_dt,			-- handling Null and Missing month,year and date characters, also changing datatype from INT to DATE
			CASE 
				WHEN sls_due_dt = 0 OR LEN(sls_due_dt) != 8 THEN NULL
				ELSE CAST(CAST(sls_due_dt AS VARCHAR) AS DATE)
			END AS sls_due_dt,			-- handling Null and Missing month,year and date characters, also changing datatype from INT to DATE
			CASE
			...
			SELECT
				REPLACE(cid, '-' , '') AS cid,		-- Replaced '-' to ''
				CASE
					WHEN UPPER(TRIM(cntry)) = 'DE' THEN 'Germany'
					WHEN UPPER(TRIM(cntry)) IN ('US','USA') THEN 'United States'
					WHEN UPPER(TRIM(cntry)) = '' OR cntry IS NULL THEN 'n/a'
					ELSE TRIM(cntry)
				END AS cntry				-- Standardise the values
